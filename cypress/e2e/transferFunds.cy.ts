...
let acc1AfterBalance: Cypress.Chainable<string>
let acc2AfterBalance: Cypress.Chainable<string>
...
it('transfer funds successfully and validate the transfer messages and sums', () => {
    ...
    cy.loginUser(myUser.username, myUser.password)
        ...
        .then(() => {
                ...
                .find('td:nth-child(2)')
                .then(($el) => {
                    cy.subtractAmount(account1.balance, parseFloat(transferSum)).then(() => {
                        expect($el.eq(0).text().replace('$', '').replace('.00', '')).to.equal(funds.returnAdd)
                    })
                        
                    // cy.addAmmount(account2.balance, parseFloat(transferSum))
                    //     .should('have.text', $el.eq(1).text().replace('$', '').replace('.00', ''))
